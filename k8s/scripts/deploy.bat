@echo off
setlocal

set "SCRIPT_DIR=%~dp0"
set "REPO_ROOT=%SCRIPT_DIR%..\.."
for %%i in ("%REPO_ROOT%") do set "REPO_ROOT=%%~fi"

echo === Music App - Deploy to Minikube ===
echo.

:: ------------------------------------------------------------------
:: 1. Check minikube is running
:: ------------------------------------------------------------------
minikube status --format="{{.Host}}" 2>nul | findstr /c:"Running" >nul 2>&1
if errorlevel 1 (
    echo ERROR: Minikube is not running.
    echo Run k8s\scripts\setup-minikube.bat first.
    exit /b 1
)
echo [OK] Minikube is running.

:: ------------------------------------------------------------------
:: 2. Build images
:: ------------------------------------------------------------------
echo [..] Building Docker images...
call "%SCRIPT_DIR%build-images.bat"
if errorlevel 1 ( echo ERROR: Image build failed & exit /b 1 )
echo [OK] Images built.

:: ------------------------------------------------------------------
:: 3. Deploy with Helm
:: ------------------------------------------------------------------
echo [..] Deploying with Helm...
helm upgrade --install music-app "%REPO_ROOT%\k8s\music-app" --namespace music-app --create-namespace --wait --timeout 10m
if errorlevel 1 ( echo ERROR: Helm deploy failed & exit /b 1 )
echo [OK] Helm release deployed.

:: ------------------------------------------------------------------
:: 4. Wait for all pods to be ready
:: ------------------------------------------------------------------
echo [..] Waiting for all pods to be ready...
kubectl wait --for=condition=Ready pods --all --namespace music-app --timeout=300s
if errorlevel 1 ( echo WARNING: Some pods may not be ready yet )
echo [OK] All pods are ready.

:: ------------------------------------------------------------------
:: 5. Print access information
:: ------------------------------------------------------------------
for /f "tokens=*" %%i in ('minikube ip') do set "MINIKUBE_IP=%%i"

echo.
echo === Deployment complete! ===
echo.
echo Minikube IP: %MINIKUBE_IP%
echo.
echo Application:
echo   Gateway:    http://music-app.local/api  (via Ingress)
echo   Tunnel:     minikube tunnel  (required for Ingress on Windows)
echo.
echo Port-forward commands:
echo   RabbitMQ:     kubectl port-forward -n music-app svc/rabbitmq 15672:15672
echo   MinIO:        kubectl port-forward -n music-app svc/minio 9001:9001
echo   Grafana:      kubectl port-forward -n music-app svc/grafana 3001:3000
echo   Prometheus:   kubectl port-forward -n music-app svc/prometheus 9090:9090
echo   Kibana:       kubectl port-forward -n music-app svc/kibana 5601:5601
echo   Jaeger:       kubectl port-forward -n music-app svc/jaeger 16686:16686
echo   Neo4j:        kubectl port-forward -n music-app svc/neo4j 7474:7474
echo.
echo Check pod status:
echo   kubectl get pods -n music-app
echo.
